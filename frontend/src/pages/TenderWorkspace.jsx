...
import { ArrowLeft, FileText, Users, AlertTriangle, Activity, Upload } from 'lucide-react'
import { tendersList } from '../data/tendersData'
...
export default function TenderWorkspace() {
  ...
  // Find the tender
  const tender = tendersList.find(t => t.id === tenderId)

  if (!tender) {
    return (
      <div className="pt-32 pb-16 px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="text-center">
          <h1 className="font-syne font-bold text-3xl text-noir-50 mb-4">Tender Not Found</h1>
    ...
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className="pt-24 pb-16 px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8 pb-6 border-b border-noir-800">
        <div className="flex items-start justify-between mb-4">
          <div>
            <span className="text-xs font-mono text-amber-500 block mb-2">{tender.id}</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-6">
          <div className="border border-noir-800 bg-noir-900/40 p-4">
            ...
            <span className="text-lg font-syne font-bold text-noir-100">{tender.estimatedValue}</span>
          </div>
          <div className="border border-noir-800 bg-noir-900/40 p-4">
            <span className="text-xs text-noir-500 font-mono block mb-1">CRITERIA</span>
            <span className="text-lg font-syne font-bold text-noir-100">{tender.extractedCriteria}</span>
          </div>
          <div className="border border-noir-800 bg-noir-900/40 p-4">
            <span className="text-xs text-noir-500 font-mono block mb-1">BIDDERS</span>
            <span className="text-lg font-syne font-bold text-noir-100">{tender.biddersCount}</span>
          </div>
          <div className="border border-noir-800 bg-noir-900/40 p-4">
            <span className="text-xs text-noir-500 font-mono block mb-1">REVIEWS</span>
            <span className={`text-lg font-syne font-bold ${tender.pendingReviews > 0 ? 'text-amber-400' : 'text-noir-100'}`}>
              {tender.pendingReviews}
            </span>
          </div>
          <div className="border border-noir-800 bg-noir-900/40 p-4">
            <span className="text-xs text-noir-500 font-mono block mb-1">PAGES</span>
            <span className="text-lg font-syne font-bold text-noir-100">{tender.totalPages}</span>
          </div>
        </div>
      </div>
